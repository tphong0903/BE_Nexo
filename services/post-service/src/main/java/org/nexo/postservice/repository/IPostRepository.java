...
public interface IPostRepository extends JpaRepository<PostModel, Long> {
    ...
    @Query(value = """
            SELECT p.* FROM post_model p
            JOIN post_hash_tag_model ph ON p.id = ph.post_id
            JOIN hash_tag_model h ON ph.hashtag_id = h.id
            ...
            countQuery = """
                    ...
                    """,
            nativeQuery = true)
    Page<PostModel> findPopularPublicPostsByHashtag(@Param("hashtag") String hashtag, Pageable pageable);
